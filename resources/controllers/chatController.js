...
module.exports = {
    getAllChats: async (req, res) => {
        try {
            ...
            if (req.user.role==='ADMIN') {
                chats = await Chat.find().populate({ path: 'users', select: '-authKey'});
            }
            if (req.user.role==='USER') {
                console.log('this is condition that user role is user user')
                allChats = await Chat.find().populate({ path: 'users', select: '-authKey'});
                console.log('this is allChats')
                console.log(allChats)
                // chats = allChats.filter(chat=> chat.users.includes(req.user.id))
                chats = allChats.filter(chat=> chat.users.some(user => user._id == req.user.id))
                console.log
            }
            res.json(chats);
        } catch (e) {
    ...
    getChat: async (req, res) => {
        try {
            ...
            if (!mongoose.Types.ObjectId.isValid(chatId)) {
                return res
                    .status(400)
                    .json({ message: "Chat format is incorrect" });;
            }
            console.log(req.user)
            let chat
            if (req.user.role==='ADMIN') {
                console.log('user role is Admin')
                chat = await Chat.findById(chatId).populate([{ path: 'users', select: '-authKey'},{path: 'messages', populate: {path: 'file', select: '-data'}}]);
                console.log(chat)
                if (!chat) {
                    console.log('this is noChat')
                    return res
                    .status(400)
                    .json({ message: "Chat is not exist" });;
                }
    ...
    createNewChat: async (req, res) => {
        try {
            ...
            return res.json({ message: "New chat create successfully", chatsList: chats });
        } catch (e) {
            console.log(e);
    ...
    deleteChat: async (req, res) => {
        try {
            ...
            const chats = await Chat.find()
            return res.json({ message: "Chat deleted successfully", chatsList: chats });
        } catch (e) {
            console.log(e);
    ...
    addUserToChat: async (req, res) => {
        try {
            ...
            if (neededChat.users.includes(userId)) {
                return res
                    .status(400)
                    .json({ message: "User with this userId already exist in this chat" });
            }
            neededChat.users.push(neededUser._id)
            await neededChat.save();
            const neededChatUpdated = await Chat.findOne({ _id:chatId }).populate('users');
            return res.json({ message: "New user to chat added successfully",
    ...
    deleteUserFromChat: async (req, res) => {
        try {
            ...
            if (!neededChat.users.includes(userId)) {
                return res
                    .status(400)
                    .json({ message: "User with this userId does not exist in this chat" });
            }
            
            neededChat.users.pull(neededUser._id)
            await neededChat.save();
            const neededChatUpdated = await Chat.findOne({ _id:chatId }).populate('users');
            return res.json({ message: "User deleted from chat successfully",
                            chat: neededChatUpdated});
        } catch (e) {
            console.log(e);
            res.status(400).json({ message: "Chat create error" });
    ...
    addUsersArrToChat: async (req, res) => {
        try {
            ...
            const neededChatUpdated = await Chat.findOne({ _id:chatId }).populate('users');
            return res.json({ message: "New users to chat added successfully",
                            chat: neededChatUpdated});
        } catch (e) {
            console.log(e);
            res.status(400).json({ message: "Chat create error" });
